		...
		return (
			<div class="col-span-1 md:col-span-2 lg:col-span-3 swiss-card p-10 flex flex-col md:flex-row gap-12 items-start group hover:border-gray-400">
				<div class="shrink-0 w-full md:w-64 h-full flex flex-col justify-between">
					<div class="w-16 h-16 bg-white border border-gray-200 rounded flex items-center justify-center mb-8">
						{brand.logo ? (
							<img
								src={brand.logo}
								alt={brand.name}
				...
				<div class="flex-1 flex flex-col justify-between h-full pt-2">
					<p class="text-gray-600 text-xl font-light leading-relaxed max-w-2xl mb-10">
						{brand.desc}
					</p>
		...
		return (
			<div class="swiss-card p-8 flex flex-col h-full hover:border-black transition-colors duration-300">
				<div class="flex justify-between items-start mb-8">
					<div class="w-16 h-10 bg-white border border-gray-200 rounded flex items-center justify-center px-2">
						{brand.logo ? (
							<img
								src={brand.logo}
								alt={brand.name}
								class="h-6 w-auto object-contain mix-blend-multiply opacity-80"
							/>
						) : (
					...
					<span class="px-2 py-1 bg-[#d3fd50] text-black rounded text-[9px] font-bold uppercase tracking-widest">
						挑战者
					</span>
				</div>
				...
				<p class="text-sm text-gray-500 leading-relaxed mb-8 font-light line-clamp-3">
					{brand.desc}
				</p>

				<div class="mt-auto pt-6 border-t border-gray-100 grid grid-cols-2 gap-4">
					<div>
						<span class="text-[9px] font-bold uppercase tracking-widest text-gray-400 block mb-1">
							产量
						</span>
	...
	return (
		<div class="swiss-card p-6 flex items-center justify-between group hover:bg-gray-50 transition-colors">
			<div class="flex items-center gap-4">
				<div class="w-10 h-10 rounded flex items-center justify-center text-xs font-bold text-gray-400 overflow-hidden bg-white border border-gray-100">
					{brand.logo ? (
						<img
							src={brand.logo}
							alt={brand.name}
				...
				<div>
					...
					<span class="text-[9px] text-gray-400 font-mono tracking-wide block max-w-[150px] truncate">
						{brand.desc}
					</span>
				</div>
			</div>
			<div class="text-right">
				...
				<span class="block text-[9px] text-gray-400 font-mono">
					目标: {brand.plan2026}
				</span>
			</div>
		</div>
